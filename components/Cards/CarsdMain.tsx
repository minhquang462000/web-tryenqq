    ...
    <div className="w-full  text-sm ">
      <div className="w-ful rounded-md text-center  flex flex-col gap-2  cursor-pointer relative h-full  m-auto">
        <div
          ...
          className=" rounded-md relative overflow-hidden w-full aspect-[3/4]    "
        >
          <Link
          ...
          <span className="text-white absolute  flex gap-1  top-2 left-2 ">
            <p className="bg-[#56ccf2] w-max px-2 py-1  rounded-md">
              {timeFormat(book?.createdAt)}
            </p>
            {book?.views > 100 && (
              <span
                style={pulseStyle}
                className="bg-[#ff2853]  py-1 px-2 rounded-md"
              >
                Hot
              </span>
            )}
        ...
        <div ref={containerRef} className="w-full h-max group relative ">
          <Link
            href={`/truyen-tranh/${convertToSlug(book?.name)}-${
              book?._id
            }.html`}
          ...
          >
            ...
            <ul className="flex gap-1 flex-wrap text-white ">
              {book?.categories.map((category: ICategory) => (
                <li
                  key={category._id}
                  className="bg-[#56ccf2] px-2 py-[2px] rounded  "
                >
                  {category.name}
                </li>
              ))}
            </ul>
            <p className="mt-2 line-clamp-5">{book?.description}</p>
          </div>
        </div>
