...
export default function SearchHeader(props: IAppProps) {
    ...
    if (keyWord.length > 3) {
      try {
        const fetchData = async () => {
          const res = await axios.get(`${DOMAIN}/api/client/books?name=${keyWord}&limit=3`);
  ...
  return (
    <div className={`border mx-3 border-[#c0c0c0]  relative dark:border-transparent dark:bg-[#3a3b3c]  flex justify-between lg:w-[450px]   px-4 py-3 rounded-full`}>
      ...
      <button className="text-[#f18121] dark:text-[#ebebeb]"
        onClick={handleChangePage}>
        <IoSearch size={18} />
      </button>
      <div className={`w-full z-[999] p-2 absolute max-h-[400px] overflow-y-auto top-[110%] shadow-[0px_1px_3px_0px_#999] right-0 bg-white dark:bg-[#3a3b3c] rounded-md
         ${keyWord.length > 0 ? "" : "hidden"}`}>
        {dataBookSearch.length <= 0 && <p className="text-sm p-3">Không Tìm Thấy Kết Quả Nào!</p>}
        {
